...
extern void transfer_vector256 (st_parameter_dt *, void *, int);
export_proto(transfer_vector256);

extern void transfer_vector256_write (st_parameter_dt *, void *, int);
export_proto(transfer_vector256_write);
...
{
#if 0
  fprintf (stderr, "unformatted_write\n");
#endif
  if (likely (dtp->u.p.current_unit->flags.convert == GFC_CONVERT_NATIVE) 
      || kind == 1)
    {
...
transfer_integer (st_parameter_dt *dtp, void *p, int kind)
{
  if ((dtp->common.flags & IOPARM_LIBRETURN_MASK) != IOPARM_LIBRETURN_OK)
    return;
/*  fprintf (stderr, "[transfer_integer] to transfer %d bytes.\n", kind); */
...
void
transfer_vector256 (st_parameter_dt *dtp, void *p, int kind)
{
  size_t size;
  int i;
  double *dp = (double *) p;
  ...
  /* In order to make it work compatibly with normal output of a single scalar
   * FP, we cut the transfer by outputting only the first FP in the vector
   * instead. Otherwise, formatting like 'E15.5, I6' may not work correctly. It
   * will encounter BT_REAL (i.e. the last 3 elements in the vector) while
   * expecting a BT_INTEGER value.  */
#if 0
  size = size_from_vector_kind (kind);
  fprintf (stderr, "to transfer %lu bytes.\n", size);
  dtp->u.p.transfer (dtp, BT_VECTOR, p, kind, size, 1);
#else
  size = size_from_vector_kind (kind) / 4;
  dtp->u.p.transfer (dtp, BT_REAL, dp, kind / 4, size, 1);
#endif

  /* The following transfers are disabled. The handling of BT_VECTOR has been
   * merged into the transfer function (formatted_transfer_scalar_write,
   * and list_formatted_write_scalar).  */
#if 0
  size = size_from_vector_kind (kind) / 4;
  dtp->u.p.transfer (dtp, BT_REAL, dp, kind / 4, size, 1);
  dtp->u.p.transfer (dtp, BT_REAL, dp + 1, kind / 4, size, 1);
  dtp->u.p.transfer (dtp, BT_REAL, dp + 2, kind / 4, size, 1);
  dtp->u.p.transfer (dtp, BT_REAL, dp + 3, kind / 4, size, 1);
#endif
}

void
transfer_vector256_write (st_parameter_dt *dtp, void *p, int kind)
{
  transfer_vector256 (dtp, p, kind);
}
