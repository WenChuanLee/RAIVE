...
void
write_v (st_parameter_dt *dtp, const fnode *f, const char *p, int len)
{
  write_float (dtp, f, p, len, 0);
}
...
static void
write_vector (st_parameter_dt *dtp, const char *source, int kind, size_t size)
{
  char semi_comma =
	dtp->u.p.current_unit->decimal_status == DECIMAL_POINT ? ',' : ';';
  ...
  if (write_char (dtp, '{'))
    return;
  write_real (dtp, source, kind / 4);

  if (write_char (dtp, semi_comma))
    return;
  write_real (dtp, source + size / 4, kind / 4);

  if (write_char (dtp, semi_comma))
    return;
  write_real (dtp, source + size / 4 * 2, kind / 4);

  if (write_char (dtp, semi_comma))
    return;
  write_real (dtp, source + size / 4 * 3, kind / 4);
