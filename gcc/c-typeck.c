			...
			tree op2_original_type)
{
  ...
  else if ((code1 == REAL_TYPE || code1 == VECTOR_TYPE)
	   && (code2 == REAL_TYPE || code2 == VECTOR_TYPE))
    {
      fprintf (stderr, "Promoting the REAL_TYPE to VECTOR_TYPE in conditional.\n");
      result_type = c_common_type (type1, type2);
      fprintf (stderr, "result_type:\n");
      debug_tree (result_type);
      do_warn_double_promotion (result_type, type1, type2,
				"implicit conversion from %qT to %qT to "
				"match other result of conditional",
				colon_loc);
  ...
  if (!result_type)
	...
	{
          fprintf (stderr, "c-typeck.c: before mismatch error\n");
...
#define WARN_FOR_QUALIFIERS(LOCATION, OPT, AR, AS, IN, RE, QUALS)        \
  ...
  /* For lineage purpose, we convert rhs to vec type */
		 ...
		 tree orig_op0, tree orig_op1, int convert_p)
{
...
#if 1
  fprintf(stderr, "code0=%s, code1=%s op=%s\n", 
      tree_code_name[code0], tree_code_name[code1], 
	  tree_code_name[code]);
#endif
  ...
  if ((code0 == INTEGER_TYPE || code0 == REAL_TYPE || code0 == COMPLEX_TYPE
       ...
       || code1 == FIXED_POINT_TYPE || code1 == VECTOR_TYPE))
    {
      ...
      bool first_vector = (code0 == VECTOR_TYPE);
      bool second_vector = (code1 == VECTOR_TYPE);
      int none_vector = (!first_vector && !second_vector);
	  ...
	  && flag_signed_zeros)
	{
	  ...
	  && (code == PLUS_EXPR
	      || code == MINUS_EXPR
	      || code == MULT_EXPR
	      || (code == TRUNC_DIV_EXPR && first_vector))
	  && TREE_CODE (TREE_TYPE (result_type)) == REAL_TYPE
	  && flag_signed_zeros)
	{
	  goto return_build_binary_op;
...
c_objc_common_truthvalue_conversion (location_t location, tree expr)
{
  ...
  switch (TREE_CODE (TREE_TYPE (expr)))
    ...
    /* NOTE: skip the checking below, since we will patch the 256-bit vector
     * predicate in later pass, e.g. -flineage-fp and -flineage. Otherwise it
     * will panic at rtl emit phase.  */
#if 0
    case VECTOR_TYPE:
