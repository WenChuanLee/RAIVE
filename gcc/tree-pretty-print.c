...
dump_function_header (FILE *dump_file, tree fdecl, int flags)
{
  ...
  fprintf (dump_file, ", return=%s", tree_code_name [TREE_CODE (TREE_TYPE (result))]);
  if (TREE_CODE (TREE_TYPE (result)) == REAL_TYPE)
  {
    fprintf (dump_file, " <%d>", TYPE_PRECISION (TREE_TYPE (result)));
  }
