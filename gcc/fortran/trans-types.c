...
#define MAX_VECTOR_KINDS 1
gfc_real_info gfc_vector_kinds[MAX_VECTOR_KINDS + 1];
static GTY(()) tree gfc_vector_types[MAX_VECTOR_KINDS + 1];

#define MAX_COMPLEXVEC_KINDS 1
gfc_real_info gfc_complexvec_kinds[MAX_COMPLEXVEC_KINDS + 1];
static GTY(()) tree gfc_complexvec_types[MAX_COMPLEXVEC_KINDS + 1];
...
int gfc_default_vector_kind;
int gfc_default_complexvec_kind;
...
gfc_init_kinds (void)
{
  ...
  for (r_index = 0, mode = MIN_MODE_VECTOR_FLOAT; mode <= MAX_MODE_VECTOR_FLOAT; mode++)
  {
    int kind;
    ...
    gfc_vector_kinds[r_index].kind = kind;
	gfc_complexvec_kinds[r_index].kind = kind << 1;
    r_index += 1;
  }
  ...
  /* The default vector kind is 32, which means a 256-bit vector containing four
   * real(kind=8) elements.  */
  gfc_default_vector_kind = 32;
  /* The default complex vector kind is 64, which means two 256-bit vectors containing 
   * eight real(kind=8) elements */
  gfc_default_complexvec_kind = 64;
...
#define PUSH_TYPE(name, node) \
  ...
  for (index = 0; gfc_real_kinds[index].kind != 0; index++)
	...
	float128_type_node = type;
      ...
      /* NOTE: kind=x means x-byte for each part; complex(8) is
       * equivalent to complex(kind=8). The size of the complex
       * variable size(v) = 16.
       *
       * complex*8 is a different thing that means the TOTAL size
       * of the variable. so size(v)=8.
       */
...
tree
gfc_get_vector_type (int kind)
{
  return build_vector_type (double_type_node, 4); /*V4DF_type_node;*/ /* double_type_node; */
}

tree
gfc_get_complexvec_type(int kind)
{
  return build_vector_type (v4df_type_node, 2); /* V4DF_type_node;*/
}
...
gfc_typenode_for_spec (gfc_typespec * spec)
{
  ...
  switch (spec->type)
    ...
    case BT_REAL:
      /* TODO: Change the default reals to vectors. but it's not
       * okay to operate this way because it will also change the intermediate
       * reals those are useful.  */
    ...
    case BT_VECTOR:
      basetype = gfc_get_vector_type (spec->kind);
      break;
    case BT_COMPLEXVEC:
      basetype = gfc_get_complexvec_type (spec->kind);
      break;
