...
gfc_expr *
gfc_simplify_dummy (gfc_expr *e)
{
  fprintf (stderr, "gfc_simplify_dummy() called. kind=%d\n", e->ts.kind);

  /* NOTE: This is not the right place to promote the type of the argument.
   * Simplification function is to detect compile-time constant values, e.g.
   * all constant arguments to a given intrinsic function, in which case it
   * returns the simplified (computed) value to its caller to replace the
   * original runtime function call.
   * Type conversion is moved to the resolve function.  */

#if 1
  if (e->expr_type == EXPR_CONSTANT && e->ts.type == BT_REAL)
  {
    gfc_expr *result;
    
    gfc_set_model_kind (e->ts.kind);
    result = gfc_get_constant_expr (BT_VECTOR, 32, &e->where);
...
gfc_expr *
gfc_simplify_dummy2 (gfc_expr *expr1, gfc_expr *expr2)
{
  fprintf (stderr, "gfc_simplify_dummy2() called.\n");
  return NULL;
}

gfc_expr *
gfc_simplify_mm256_get_pd(gfc_expr *expr1, gfc_expr *expr2)
{
	/* We don't handle here because expr1 must not be constant */
	return NULL;
}

gfc_expr *
gfc_simplify_mm256_set_pd(gfc_expr *expr1, gfc_expr *expr2, gfc_expr *expr3)
{
	/* We don't handle here because expr1 must not be constant */
	return NULL;
}
