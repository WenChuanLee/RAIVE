...
gfc_set_model_kind (int kind)
{
  ...
  fprintf (stderr, "gfc_set_model_kind: %d\n", base2prec);
...
gfc_range_check (gfc_expr *e)
{
  ...
  switch (e->ts.type)
    ...
    case BT_VECTOR:
      /* TODO: skipping range check for now.  */
      rc = ARITH_OK;
      break;
...
gfc_arith_uminus (gfc_expr *op1, gfc_expr **resultp)
{
  ...
  switch (op1->ts.type)
    ...
    case BT_VECTOR:
      mpfr_neg (result->value.vector.elem[0], op1->value.vector.elem[0], GFC_RND_MODE);
      mpfr_neg (result->value.vector.elem[1], op1->value.vector.elem[1], GFC_RND_MODE);
      mpfr_neg (result->value.vector.elem[2], op1->value.vector.elem[2], GFC_RND_MODE);
      mpfr_neg (result->value.vector.elem[3], op1->value.vector.elem[3], GFC_RND_MODE);
      break;
...
gfc_arith_minus (gfc_expr *op1, gfc_expr *op2, gfc_expr **resultp)
{
  ...
  switch (op1->ts.type)
    ...
    case BT_VECTOR:
      mpfr_sub (result->value.vector.elem[0], op1->value.vector.elem[0], op2->value.vector.elem[0],
		GFC_RND_MODE);
      mpfr_sub (result->value.vector.elem[1], op1->value.vector.elem[1], op2->value.vector.elem[1],
		GFC_RND_MODE);
      mpfr_sub (result->value.vector.elem[2], op1->value.vector.elem[2], op2->value.vector.elem[2],
		GFC_RND_MODE);
      mpfr_sub (result->value.vector.elem[3], op1->value.vector.elem[3], op2->value.vector.elem[3],
		GFC_RND_MODE);
      break;
...
gfc_arith_times (gfc_expr *op1, gfc_expr *op2, gfc_expr **resultp)
{
  ...
  switch (op1->ts.type)
    ...
    case BT_VECTOR:
      mpfr_mul (result->value.vector.elem[0], op1->value.vector.elem[0], op2->value.vector.elem[0],
	       GFC_RND_MODE);
      mpfr_mul (result->value.vector.elem[1], op1->value.vector.elem[1], op2->value.vector.elem[1],
	       GFC_RND_MODE);
      mpfr_mul (result->value.vector.elem[2], op1->value.vector.elem[2], op2->value.vector.elem[2],
	       GFC_RND_MODE);
      mpfr_mul (result->value.vector.elem[3], op1->value.vector.elem[3], op2->value.vector.elem[3],
	       GFC_RND_MODE);
      break;
...
gfc_arith_divide (gfc_expr *op1, gfc_expr *op2, gfc_expr **resultp)
{
  ...
  switch (op1->ts.type)
    ...
    case BT_VECTOR:
      mpfr_div (result->value.vector.elem[0], op1->value.vector.elem[0], op2->value.vector.elem[0],
	       GFC_RND_MODE);
      mpfr_div (result->value.vector.elem[1], op1->value.vector.elem[1], op2->value.vector.elem[1],
	       GFC_RND_MODE);
      mpfr_div (result->value.vector.elem[2], op1->value.vector.elem[2], op2->value.vector.elem[2],
	       GFC_RND_MODE);
      mpfr_div (result->value.vector.elem[3], op1->value.vector.elem[3], op2->value.vector.elem[3],
	       GFC_RND_MODE);
      break;
		...
		eval_f eval, gfc_expr *op1, gfc_expr *op2)
{
  ...
  switch (op)
    ...
    case INTRINSIC_POWER:
      fprintf (stderr, "[arith.c] op1=%s, op2=%s\n", gfc_typename (&op1->ts),
          gfc_typename (&op2->ts));
#if 0
      gfc_debug_expr (op1);
      fprintf (stderr, "\n");
...
/* Convert a tuple of four real, constant expression nodes to a single
   vector expression node.  */

gfc_expr *
gfc_convert_vector (gfc_expr *elem0, gfc_expr *elem1, gfc_expr *elem2, gfc_expr *elem3, int kind)
{
  gfc_expr *e;

  e = gfc_get_constant_expr (BT_VECTOR, kind, &elem0->where);
  mpfr_set (e->value.vector.elem[0], elem0->value.real, GFC_RND_MODE);
  mpfr_set (e->value.vector.elem[1], elem1->value.real, GFC_RND_MODE);
  mpfr_set (e->value.vector.elem[2], elem2->value.real, GFC_RND_MODE);
  mpfr_set (e->value.vector.elem[3], elem3->value.real, GFC_RND_MODE);
...
/* Convert default integer to default vector.  */

gfc_expr *
gfc_int2vector (gfc_expr *src, int kind)
{
  gfc_expr *result;

  fprintf (stderr, "int2vector: kind=%d\n", kind);

  result = gfc_get_constant_expr (BT_VECTOR, kind, &src->where);

  mpfr_set_z (result->value.vector.elem[0], src->value.integer, GFC_RND_MODE);
  mpfr_set_z (result->value.vector.elem[1], src->value.integer, GFC_RND_MODE);
  mpfr_set_z (result->value.vector.elem[2], src->value.integer, GFC_RND_MODE);
  mpfr_set_z (result->value.vector.elem[3], src->value.integer, GFC_RND_MODE);
...
/* Convert default integer to default complex vector */
gfc_expr *
gfc_int2complexvec (gfc_expr *src, int kind)
{
  gfc_expr *result;
  arith rc = 0;

  result = gfc_get_constant_expr (BT_COMPLEXVEC, kind, &src->where);

  mpc_set_z (result->value.complexvec.elem[0], 
			src->value.integer, GFC_MPC_RND_MODE);
  mpc_set_z (result->value.complexvec.elem[1], 
			src->value.integer, GFC_MPC_RND_MODE);
  mpc_set_z (result->value.complexvec.elem[2], 
			src->value.integer, GFC_MPC_RND_MODE);
  mpc_set_z (result->value.complexvec.elem[3], 
			src->value.integer, GFC_MPC_RND_MODE);
  
  rc += gfc_check_real_range (mpc_realref (
			  result->value.complexvec.elem[0]), 8);
  rc += gfc_check_real_range (mpc_realref (
			  result->value.complexvec.elem[1]), 8);
  rc += gfc_check_real_range (mpc_realref (
			  result->value.complexvec.elem[2]), 8);
  rc += gfc_check_real_range (mpc_realref (
			  result->value.complexvec.elem[3]), 8);
...
/* Convert real to default vector.  */

gfc_expr *
gfc_real2vector (gfc_expr *src, int kind)
{
  gfc_expr *result;

  result = gfc_get_constant_expr (BT_VECTOR, kind, &src->where);
  fprintf (stderr, "[gfc_real2vector] kind=%d\n", kind);
  mpfr_prec_t p = mpfr_get_prec (src->value.real);
  mpfr_fprintf (stderr, "variable x with %Pu bits\n", p);
  fprintf (stderr, "Sum is:");
  mpfr_out_str (stderr, 10, 0, src->value.real, MPFR_RNDD);
  fprintf (stderr, "\n");

  mpfr_set (result->value.vector.elem[0], src->value.real, GFC_RND_MODE);
  mpfr_set (result->value.vector.elem[1], src->value.real, GFC_RND_MODE);
  mpfr_set (result->value.vector.elem[2], src->value.real, GFC_RND_MODE);
  mpfr_set (result->value.vector.elem[3], src->value.real, GFC_RND_MODE);
...
gfc_expr *
gfc_real2complexvec (gfc_expr *src, int kind)
{
  gfc_expr *result;
  arith rc = 0;
  ...
  mpc_set_fr (
		result->value.complexvec.elem[0], src->value.real, GFC_MPC_RND_MODE);
  mpc_set_fr (
		result->value.complexvec.elem[1], src->value.real, GFC_MPC_RND_MODE);
  mpc_set_fr (
		result->value.complexvec.elem[2], src->value.real, GFC_MPC_RND_MODE);
  mpc_set_fr (
		result->value.complexvec.elem[3], src->value.real, GFC_MPC_RND_MODE);

  rc += gfc_check_real_range (
		  mpc_realref (result->value.complexvec.elem[0]), 8);
  rc += gfc_check_real_range (
		  mpc_realref (result->value.complexvec.elem[1]), 8);
  rc += gfc_check_real_range (
		  mpc_realref (result->value.complexvec.elem[2]), 8);
  rc += gfc_check_real_range (
		  mpc_realref (result->value.complexvec.elem[3]), 8);

  if (rc == ARITH_UNDERFLOW * 4)
    {
      if (gfc_option.warn_underflow)
	gfc_warning (gfc_arith_error (rc), &src->where);
      mpfr_set_ui (mpc_realref (result->value.complex), 0, GFC_RND_MODE);
    }
  else if (rc != ARITH_OK * 4)
    {
      arith_error (rc, &src->ts, &result->ts, &src->where);
      gfc_free_expr (result);
      return NULL;
