...
/* Get a new expression node that is a vector constructor
   of given type and kind.  */

gfc_expr *
gfc_get_vector_expr (bt type, int kind, locus *where)
{
  gfc_expr *e;

  e = gfc_get_expr ();
  e->expr_type = EXPR_VECTOR;
  e->value.constructor = NULL;
  e->rank = 0;
  e->shape = NULL;
...
gfc_get_constant_expr (bt type, int kind, locus *where)
{
  ...
  switch (type)
    ...
    case BT_VECTOR:
      /* We are not setting model_kind here, which will set the default prec
       * of mpfr.  
       * Fixed on Oct.18th, 2013. */
      fprintf (stderr, "BT_VECTOR: kind=%d\n", kind);
      if (kind == 32)
      {
        kind = 8;
      }
      ...
      mpfr_init (e->value.vector.elem[0]);
      mpfr_init (e->value.vector.elem[1]);
      mpfr_init (e->value.vector.elem[2]);
      mpfr_init (e->value.vector.elem[3]);
      break;
	
	case BT_COMPLEXVEC:
      fprintf (stderr, "BT_COMPLEXVEC: kind=%d\n", kind);
      /* Don't know why */
	  gfc_set_model_kind (8);

      mpc_init2 (e->value.complexvec.elem[0], mpfr_get_default_prec());
      mpc_init2 (e->value.complexvec.elem[1], mpfr_get_default_prec());
      mpc_init2 (e->value.complexvec.elem[2], mpfr_get_default_prec());
      mpc_init2 (e->value.complexvec.elem[3], mpfr_get_default_prec());
	  break;
...
{
  return type == BT_VECTOR || type == BT_COMPLEX || type == BT_REAL || type == BT_INTEGER;
}
...
vector_type (bt type)
{
  return type == BT_VECTOR;
}

int
gfc_vector_ts (gfc_typespec *ts)
{
  return vector_type (ts->type);
}
...
gfc_type_convert_binary (gfc_expr *e, int wconversion)
{
  ...
  /* Real combined with vector.  */
  if (op2->ts.type == BT_VECTOR)
  {
    e->ts = op2->ts;
    gfc_convert_type_warn (e->value.op.op1, &e->ts, 2, wconversion);
    goto done;
...
gfc_reduce_init_expr (gfc_expr *expr)
{
  ...
  if (expr->expr_type == EXPR_VECTOR)
  {
    if (gfc_expand_constructor (expr, true) == FAILURE)
      return FAILURE;

    fprintf (stderr, "reduced EXPR_VECTOR\n");
  }
...
gfc_check_assign (gfc_expr *lvalue, gfc_expr *rvalue, int conform)
{
  ...
  if (lvalue->ts.type == BT_VECTOR && rvalue->expr_type == EXPR_VECTOR)
    return SUCCESS;
