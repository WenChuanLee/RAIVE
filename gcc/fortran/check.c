...
/* Make sure an expression is constant or not */

static gfc_try
const_check (gfc_expr *e, int n, bool isConst)
{
  if (isConst) {
	...
	if (e->expr_type == EXPR_CONSTANT) {
		gfc_error ("'%s' argument of '%s' intrinsic at %L must not be a constant",
				gfc_current_intrinsic_arg[n]->name, gfc_current_intrinsic,
				&e->where);
		return FAILURE;
	}
...
static gfc_try
real_or_complex_or_vector_check (gfc_expr *e, int n)
{
  if (e->ts.type != BT_REAL && e->ts.type != BT_COMPLEX && e->ts.type != BT_VECTOR)
    {
      gfc_error ("'%s' argument of '%s' intrinsic at %L must be REAL "
		 "or COMPLEX or VECTOR", gfc_current_intrinsic_arg[n]->name,
		 gfc_current_intrinsic, &e->where);
      return FAILURE;
    }
...
/* Check that an expression is vector.
 * We allow BT_REAL and BT_INTEGER if they are constants.  */

static gfc_try
vector_check (gfc_expr *e, int n)
{
  if ((e->ts.type != BT_VECTOR) &&
      ((e->ts.type == BT_REAL && e->expr_type != EXPR_CONSTANT) ||
      (e->ts.type == BT_INTEGER && e->expr_type != EXPR_CONSTANT)))
  {
    gfc_error ("'%s' argument of '%s' intrinsic at %L must be VECTOR",
...
/* A single real or complex or vector argument.  */

gfc_try
gfc_check_fn_rcv (gfc_expr *a)
{
  if (real_or_complex_or_vector_check (a, 0) == FAILURE)
    return FAILURE;
...
gfc_try
gfc_check_fn_v (gfc_expr *a)
{
  fprintf (stderr, "check_fn_v: %s\n", 
      gfc_basic_typename (a->ts.type));

  if (vector_check (a, 0) == FAILURE)
    return FAILURE;
...
gfc_try
gfc_check_fn_v_v (gfc_expr *a, gfc_expr *b)
{
  fprintf (stderr, "check_fn_v_v: %s, %s\n", 
      gfc_basic_typename (a->ts.type), gfc_basic_typename (b->ts.type));
  if (vector_check (a, 0) == FAILURE || vector_check (b, 0) == FAILURE)
    return FAILURE;

  return SUCCESS;
}

/* One VECTOR variable and one Integer constant */
gfc_try
gfc_check_mm256_get_pd (gfc_expr *a, gfc_expr *b)
{
  int idx;

  /* This is vector with size 256 bits */
  if (type_check (a, 0, BT_VECTOR) == FAILURE ||
	  const_check(b, 1, true) == FAILURE ||
	  type_check (b, 1, BT_INTEGER) == FAILURE)
	  return FAILURE;

  gfc_extract_int(b, &idx);

  if (idx > 3 || idx < 0) {
	  gfc_error ("'%s' argument of '%s' intrinsic at %L index out of range",
		gfc_current_intrinsic_arg[1]->name, gfc_current_intrinsic,
		&b->where);
	  return FAILURE;
  }
...
/* 
 * One Vector variable, one Integer constant and 
 * one Real*8 (constant/variable) 
 */
gfc_try
gfc_check_mm256_set_pd (gfc_expr *a, gfc_expr *b, gfc_expr *c)
{
  int idx;

  /* This is vector with size 256 bits */
  if (type_check (a, 0, BT_VECTOR) == FAILURE ||
	  const_check (a, 0, false) == FAILURE ||
	  type_check (b, 1, BT_INTEGER) == FAILURE ||
	  int_or_real_check(c, 2) == FAILURE)
	  return FAILURE;
