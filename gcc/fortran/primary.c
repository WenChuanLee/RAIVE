...
/* Try to match a vector constant.  */

static match
match_vector_constant (gfc_expr **result)
{
  gfc_expr *e, *elem0, *elem1, *elem2, *elem3;
  gfc_error_buf old_error;
  locus old_loc;
  ...
  old_loc = gfc_current_locus;
  e = elem0 = elem1 = elem2 = elem3 = NULL;
  ...
  kind = gfc_default_vector_kind;

  e = gfc_convert_vector (elem0, elem1, elem2, elem3, kind);
  e->where = gfc_current_locus;
...
syntax:
  gfc_error ("Syntax error in VECTOR constant at %C");
  m = MATCH_ERROR;
...
gfc_match_literal_constant (gfc_expr **result, int signflag)
{
  ...
  m = match_vector_constant (result);
  if (m != MATCH_NO)
    return m;
