...
static const mstring bt_types[] = {
    ...
    minit ("VECTOR", BT_VECTOR),
    minit ("COMPLEXVEC", BT_COMPLEXVEC),
...
mio_expr (gfc_expr **ep)
{
  ...
  switch (e->expr_type)
	...
	case BT_VECTOR:
	  fprintf (stderr, "[module] kind=%d\n", e->ts.kind);
	  gfc_set_model_kind (e->ts.kind / 4);
	  mio_gmp_real (&e->value.vector.elem[0]);
	  mio_gmp_real (&e->value.vector.elem[1]);
	  mio_gmp_real (&e->value.vector.elem[2]);
	  mio_gmp_real (&e->value.vector.elem[3]);
	  break;
