...
convert (tree type, tree expr)
{
  ...
  /* This is different from the original gfc code.  */
#if 0
  if (code == VECTOR_TYPE)
  ...
  if (code == VECTOR_TYPE)
  {
      ...
      fprintf (stderr, "[convert.c] build new ctor: \n");
      debug_tree (e);
      debug_tree (ctor);

      return fold (ctor);
#else
      tree elem = fold (convert_to_real (double_type_node, e));
      ctor = fold (build_vector_from_val (v4df_type_node, elem));
