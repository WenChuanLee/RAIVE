...
#include "../config/i386/ix86-builtins.h"
				  ...
				  tree *argarray, int nargs)
{
  ...
  fprintf (stderr, "in gfc_conv_intrinsic_function_args: nargs=%d\n", nargs);
...
gfc_conv_intrinsic_conversion (gfc_se * se, gfc_expr * expr)
{
  ...
  fprintf (stderr, "entering gfc_conv_intrinsic_conversion.\n");
  ...
  if (expr->ts.type == BT_CHARACTER)
	...
	gcc_unreachable ();
      ...
      fprintf (stderr, "leaving gfc_conv...\n");
      return;
    }
  ...
  fprintf (stderr, "before calling convert:\n");
  debug_tree (type);
  debug_tree (args[0]);
  se->expr = convert (type, args[0]);
  fprintf (stderr, "after calling convert.\n");
}
...
static void
gfc_conv_intrinsic_sqrtpd256 (gfc_se *se, gfc_expr * expr)
{
  tree arg;
  tree arg1;
  tree sqrtpd256;
  ...
  sqrtpd256 = targetm.builtin_decl (IX86_BUILTIN_SQRTPD256, 0);

  fprintf (stderr, "in gfc_conv_intrinsic_sqrtpd256\n");
  debug_tree (sqrtpd256);

  arg1 = arg;

  if (TREE_CODE (TREE_TYPE (arg1)) == REAL_TYPE)
  {
    arg1 = fold (build_vector_from_val (v4df_type_node, arg));
  }

  resvar = build_call_expr_loc (input_location, sqrtpd256, 1, arg1);

  se->expr = resvar;
}

static void
gfc_conv_intrinsic_maxpd256 (gfc_se *se, gfc_expr * expr)
{
  tree *args;
  tree maxpd256;
  tree arg1, arg2;
  ...
  if (nargs != 2)
  {
    gfc_error ("Number of arguments of %s intrinsic at %L is invalid.",
        expr->value.function.isym->name,&expr->where);
  }
  ...
  maxpd256 = targetm.builtin_decl (IX86_BUILTIN_MAXPD256, 0);

  fprintf (stderr, "maxpd:");
  debug_tree (maxpd256);
  ...
  if (TREE_CODE (TREE_TYPE (arg1)) == REAL_TYPE)
  {
    arg1 = fold (build_vector_from_val (v4df_type_node, args[0]));
  }

  if (TREE_CODE (TREE_TYPE (arg2)) == REAL_TYPE)
  {
    arg2 = fold (build_vector_from_val (v4df_type_node, args[1]));
  }

  resvar = build_call_expr_loc (input_location, maxpd256, 2, arg1, arg2);

  se->expr = resvar;
}

static void
gfc_conv_intrinsic_minpd256 (gfc_se *se, gfc_expr * expr)
{
  tree *args;
  tree minpd256;
  tree arg1, arg2;
  ...
  minpd256 = targetm.builtin_decl (IX86_BUILTIN_MINPD256, 0);
  ...
  if (TREE_CODE (TREE_TYPE (arg1)) == REAL_TYPE)
  {
    arg1 = fold (build_vector_from_val (v4df_type_node, arg1));
  }

  if (TREE_CODE (TREE_TYPE (arg2)) == REAL_TYPE)
  {
    arg2 = fold (build_vector_from_val (v4df_type_node, arg2));
  }

  resvar = build_call_expr_loc (input_location, minpd256, 2, arg1, arg2);
 ...
 * a = *((double *)&m[i]);
 *
 * Note: i must be a constant and 0 <= i <= 3
 */
static void
gfc_conv_intrinsic_mm256_get_pd (gfc_se *se, gfc_expr * expr)
{
  tree *args;
  int nargs;
  tree tmp;
  ...
  tmp = fold_build1_loc (input_location, ADDR_EXPR, 
		  build_pointer_type(v4df_type_node), args[0]);

  tmp = fold_build1_loc (input_location, CONVERT_EXPR, 
		  double_ptr_type_node, tmp);

  tmp = fold_build2_loc (input_location, POINTER_PLUS_EXPR, 
		  double_ptr_type_node, tmp, 
		  build_int_cst(long_unsigned_type_node, TREE_INT_CST_LOW(args[1])<<3));

  tmp = fold_build1_loc (input_location, INDIRECT_REF, double_type_node, tmp);
 ...
 * *((double *)&m[i]) = x;
 *
 * Note 1: i must be a constant and 0 <= i <= 3
 * Note 2: x can be a constant or a variable
 */
static void
gfc_conv_intrinsic_mm256_set_pd (gfc_se *se, gfc_expr * expr)
{
  tree *args;
  int nargs;
  tree tmp, tmp2;
  tree ptr = gfc_create_var (double_ptr_type_node, "ptr");
  ...
  gfc_add_modify (&se->pre, ptr, tmp);
  
  tmp = fold_build1_loc (input_location, INDIRECT_REF, double_type_node, ptr);

  gfc_add_modify (&se->pre, tmp, convert(double_type_node, args[2]));
...
/* NOTE: The following builtin method is not correct. The builtins are already
 * created during the initialization phase. It is not necessary to rebuild a
 * redundant copy.  */

#if 0
static tree
define_ix86_builtin (const char *name, tree type, bool is_const)
{
  tree fndecl;
...
#if 0
  /* mm functions come here */
  {
    tree mm256_sqrtv_fntype;
    
#define build_function_type_1(rtype, arg1)          \
  build_function_type (rtype, tree_cons (0, arg1, void_list_node))

    mm256_sqrtv_fntype = 
        build_function_type_1 (v4df_type_node, v4df_type_node);

    mm256_sqrtv_fndecl = targetm.builtin_decl (599, 0);
/*        define_ix86_builtin ("_mm256_sqrt_pd", mm256_sqrtv_fntype, 1); */
/*        define_ix86_builtin ("__builtin_ia32_sqrtpd256", mm256_sqrtv_fntype, 1); */
...
gfc_conv_intrinsic_function (gfc_se * se, gfc_expr * expr)
{
  ...
  fprintf (stderr, "gfc_conv_intrinsic_function: id=%d, sqrtpd=%d, conversion=%d\n", 
		expr->value.function.isym->id, GFC_ISYM_SQRTPD256, GFC_ISYM_CONVERSION);
  ...
  switch (expr->value.function.isym->id)
    ...
    case GFC_ISYM_SQRTPD256:
      gfc_conv_intrinsic_sqrtpd256 (se, expr);
      break;

    case GFC_ISYM_MAXPD256:
      gfc_conv_intrinsic_maxpd256 (se, expr);
      break;

    case GFC_ISYM_MINPD256:
      gfc_conv_intrinsic_minpd256 (se, expr);
      break;
    ...
    case GFC_ISYM_YN2:
      gfc_conv_intrinsic_funcall (se, expr);
      break;
	case GFC_ISYM_MM256_GET_PD:
      gfc_conv_intrinsic_mm256_get_pd (se, expr);
	  break;
	case GFC_ISYM_MM256_SET_PD:
      gfc_conv_intrinsic_mm256_set_pd (se, expr);
	  break;
    case GFC_ISYM_EOSHIFT:
    case GFC_ISYM_PACK:
    case GFC_ISYM_RESHAPE:
