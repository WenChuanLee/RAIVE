...
gfc_conv_constant_to_tree (gfc_expr * expr)
{
  ...
  switch (expr->ts.type)
    ...
    case BT_VECTOR:
    {
      ...
      int kind = expr->ts.kind / nunits;
      
      elem[0] = gfc_conv_mpfr_to_tree (expr->value.vector.elem[0],
          kind, expr->is_snan);
      elem[1] = gfc_conv_mpfr_to_tree (expr->value.vector.elem[1],
          kind, expr->is_snan);
      elem[2] = gfc_conv_mpfr_to_tree (expr->value.vector.elem[2],
          kind, expr->is_snan);
      elem[3] = gfc_conv_mpfr_to_tree (expr->value.vector.elem[3],
          kind, expr->is_snan);

      fprintf (stderr, "before generating VECTOR_CST, nunits=%d\n", nunits);

      v = VEC_alloc (constructor_elt, gc, nunits);
      CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, elem[0]);
      CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, elem[1]);
      CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, elem[2]);
      CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, elem[3]);

      ctor = build_vector_from_ctor (vectype, v);
