...
gfc_build_io_library_fndecls (void)
{
  ...
  /* AVX 256-bit vector */
  iocall[IOCALL_X_VECTOR256_WRITE] = gfc_build_library_function_decl_with_spec (
	get_identifier (PREFIX("transfer_vector256_write")), ".wR",
	void_type_node, 3, dt_parm_type, pvoid_type_node, gfc_int4_type_node);
...
transfer_expr (gfc_se * se, gfc_typespec * ts, tree addr_expr, gfc_code * code)
{
  ...
  switch (ts->type)
	...
	{
	  function = iocall[IOCALL_X_VECTOR256_WRITE];
	}
