...
void
gfc_resolve_sqrtv (gfc_expr *f, gfc_expr *x)
{
  f->ts = x->ts;
  f->value.function.name
    = gfc_get_string ("__sqrtv_%c%d", gfc_type_letter (x->ts.type), x->ts.kind);
}


void
gfc_resolve_dummy (gfc_expr *f, gfc_expr *x)
{
  f->value.function.name = gfc_get_string ("__dummy");

  if (x->expr_type == EXPR_CONSTANT && x->ts.type == BT_REAL)
  {
    gfc_expr *result;
    
    gfc_set_model_kind (x->ts.kind);
    result = gfc_get_constant_expr (BT_VECTOR, 32, &x->where);
    
    f->ts = result->ts;

#if 0
    mpfr_set (result->value.vector.elem[0], e->value.real, GFC_RND_MODE);
    mpfr_set (result->value.vector.elem[1], e->value.real, GFC_RND_MODE);
    mpfr_set (result->value.vector.elem[2], e->value.real, GFC_RND_MODE);
    mpfr_set (result->value.vector.elem[3], e->value.real, GFC_RND_MODE);

    fprintf (stderr, "new result generated.\n");
#endif
  }
  else
  {
...
void
gfc_resolve_dummy2 (gfc_expr *f, gfc_expr *x, gfc_expr *y)
{
  /* We need to guarantee the type of x->ts is indeed the one we need. In case
   * of constant argument, we need to promote it to vector type.  */
  ...
  {
    gcc_assert (x->ts.type == BT_VECTOR);
    f->ts = x->ts;
  }

  /* NOTE: It seems any dummy string may play the trick. The actual intrinsic
   * function will be emitted at later phase, e.g. in
   * gfc_conv_intrinsic_sqrtpd256() for sqrtpd256.  */
  f->value.function.name = gfc_get_string ("__dummy");
}

void gfc_resolve_mm256_get_pd(gfc_expr *f, gfc_expr *x, gfc_expr *y)
{
  ...
  f->value.function.name = 
	  gfc_get_string ("mm256_get_pd");
}

void gfc_resolve_mm256_set_pd(gfc_expr *f, gfc_expr *x, gfc_expr *y, gfc_expr *z)
{

  f->ts = x->ts;
  
  f->value.function.name = 
	  gfc_get_string ("mm256_set_pd");
}
