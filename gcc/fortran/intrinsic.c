...
find_conv (gfc_typespec *from, gfc_typespec *to)
{
  ...
  fprintf (stderr, "[find_conv] target=%s, sym=", target);
...
add_functions (void)
{
  ...
  int di, dr, dv, dd, dl, dc, dz, ii;

  di = gfc_default_integer_kind;
  dr = gfc_default_real_kind;
  dv = gfc_default_vector_kind;
  ...
  /* builtin functions for immintrin.h */

  add_sym_1 ("mm256_sqrt_pd", GFC_ISYM_SQRTPD256, CLASS_ELEMENTAL, ACTUAL_YES, BT_VECTOR, dv,
             GFC_STD_F77, gfc_check_fn_v, gfc_simplify_dummy, gfc_resolve_dummy,
	     x, BT_VECTOR, dv, REQUIRED);

  add_sym_2 ("mm256_max_pd", GFC_ISYM_MAXPD256, CLASS_ELEMENTAL, ACTUAL_YES, BT_VECTOR, dv, 
             GFC_STD_F77, gfc_check_fn_v_v, gfc_simplify_dummy2, gfc_resolve_dummy2,
	     x, BT_VECTOR, dv, REQUIRED, x, BT_VECTOR, dv, REQUIRED);

  add_sym_2 ("mm256_min_pd", GFC_ISYM_MINPD256, CLASS_ELEMENTAL, ACTUAL_YES, BT_VECTOR, dv,
			GFC_STD_F77, gfc_check_fn_v_v, gfc_simplify_dummy2, gfc_resolve_dummy2,
	     x, BT_VECTOR, dv, REQUIRED, x, BT_VECTOR, dv, REQUIRED);

  /* Add support for vector type */
  add_sym_2 ("mm256_get_pd", GFC_ISYM_MM256_GET_PD, CLASS_ELEMENTAL, ACTUAL_YES, BT_REAL, 
		 dd, GFC_STD_F77, gfc_check_mm256_get_pd, gfc_simplify_mm256_get_pd, 
		 gfc_resolve_mm256_get_pd, 
		 x, BT_VECTOR, dv, REQUIRED, x, BT_INTEGER, di, REQUIRED);

  add_sym_3 ("mm256_set_pd", GFC_ISYM_MM256_SET_PD, CLASS_ELEMENTAL, ACTUAL_YES, BT_VECTOR, 
		 dv, GFC_STD_F77, gfc_check_mm256_set_pd, gfc_simplify_mm256_set_pd, 
		 gfc_resolve_mm256_set_pd, 
		 x, BT_VECTOR, dv, REQUIRED, x, BT_INTEGER, di, REQUIRED, x, BT_REAL, dd, REQUIRED);
...
add_conversions (void)
{
  ...
  for (i = 0; gfc_integer_kinds[i].kind != 0; i++)
  {
    ...
    add_conv (BT_INTEGER, gfc_integer_kinds[i].kind,
        BT_VECTOR, gfc_vector_kinds[0].kind, GFC_STD_F77);
    
	add_conv (BT_INTEGER, gfc_integer_kinds[i].kind,
        BT_COMPLEXVEC, gfc_complexvec_kinds[0].kind, GFC_STD_F77);
  }
...
gfc_convert_type_warn (gfc_expr *expr, gfc_typespec *ts, int eflag, int wflag)
{
  ...
  fprintf (stderr, "gfc_convert_type_warn1: wflag=%d, from=%s, to=%s\n", 
		wflag, gfc_typename (&from_ts), gfc_typename (ts));

  if (expr->ts.type == BT_DERIVED && ts->type == BT_DERIVED
      && gfc_compare_types (&expr->ts, ts))
    return SUCCESS;

  fprintf (stderr, "gfc_convert_type_warn2: wflag=%d\n", wflag); 

  sym = find_conv (&expr->ts, ts);
  if (sym == NULL)
    goto bad;

  fprintf (stderr, "gfc_convert_type_warn3: wflag=%d\n", wflag);
      ...
      else if (from_ts.type == BT_LOGICAL || ts->type == BT_LOGICAL)
	{
	  /* Do nothing. This block exists only to simplify the other
	       ...
	       || (from_ts.type == BT_INTEGER && ts->type == BT_COMPLEX)
               || (from_ts.type == BT_INTEGER && ts->type == BT_VECTOR)
               || (from_ts.type == BT_INTEGER && ts->type == BT_COMPLEXVEC)
	       || (from_ts.type == BT_REAL && ts->type == BT_COMPLEX)
	       || (from_ts.type == BT_REAL && ts->type == BT_VECTOR)
	       || (from_ts.type == BT_REAL && ts->type == BT_COMPLEXVEC))
	{
          fprintf (stderr, "[intrinsic.c] Conversion from %s to %s\n", 
              gfc_typename (&from_ts), gfc_typename (ts));
  ...
  *expr = *new_expr;
  ...
  fprintf (stderr, "[intrinsic.c] leaving.\n");
  return SUCCESS;
