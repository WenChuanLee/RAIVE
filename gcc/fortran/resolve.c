...
resolve_symbol (gfc_symbol *sym)
{
  ...
  if (sym->ts.type == BT_UNKNOWN)
      ...
      if (sym->attr.flavor == FL_PROCEDURE && sym->attr.function)
	{
	  ...
	  if (!mp_flag) {
	    /* 
		 * This function should has its own namespace, so we search
		 * the global namespace list to get the global procedure's
		 * type
		 */
		for (ns=gfc_global_ns_list; ns; ns=ns->sibling) {
			gfc_symbol *nSym = ns->proc_name;

			if (strcmp(nSym->name, sym->name) == 0 &&
				nSym->attr.flavor == FL_PROCEDURE &&
				nSym->attr.function) 
			{
				sym->ts = nSym->ts;
				break;	
