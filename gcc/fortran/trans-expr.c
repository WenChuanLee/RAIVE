		      ...
		      bool array, bool pointer, bool procptr)
{
  ...
  /* Make it fall through to default case, in which it will call
   * gfc_conv_constant to handle it. */
  /*
  else if (expr->ts.type == BT_VECTOR)
  {
    tree ctor;
    ...
    fprintf (stderr, "before calling array_initilaizer, nunits=%d\n", nunits);

    v = VEC_alloc (constructor_elt, gc, nunits);
    CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, build_real (double_type_node, dconst2));
    CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, build_real (double_type_node, dconst2));
    CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, build_real (double_type_node, dconst2));
    CONSTRUCTOR_APPEND_ELT (v, NULL_TREE, build_real (double_type_node, dconst2));
...
{
  fprintf (stderr, "[gfc_trans_assign]:\n");
#if 0 
  fprintf (stderr, "op1: ");
  gfc_debug_expr (code->expr1);
  fprintf (stderr, "op2: ");
  gfc_debug_expr (code->expr2);
  fprintf (stderr, "\n");
#endif
