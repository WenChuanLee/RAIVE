...
gfc_match_data (void)
{
  ...
  fprintf (stderr, "in gfc_match_data\t");
	   ...
	   gfc_array_spec **as, locus *var_locus)
{
  ...
  fprintf (stderr, "build_sym: [name '%s']", name);
  if (sym->ts.type == BT_REAL)
  {
    fprintf (stderr, " [BT_REAL]kind=%d", sym->ts.kind);

    /* sym->ts.type = BT_VECTOR; */
  }
  else if (sym->ts.type == BT_COMPLEX)
  {
    fprintf (stderr, " [BT_COMPLEX]");
  }
  else if (sym->ts.type == BT_VECTOR)
  {
    fprintf (stderr, " [BT_VECTOR]");
  }
  else if (sym->ts.type == BT_COMPLEXVEC)
  {
    fprintf (stderr, " [BT_COMPLEXVEC]");
  }
  else
  {
	      ...
	      gfc_array_spec **as)
{
  ...
  *as = NULL;
  ...
  if (flag_lineage_vec)
  {
    if (c->ts.type == BT_REAL)
    {
      fprintf (stderr, "[build_struct][BT_REAL][kind=%d] %s\n", c->ts.kind, name);
    }
  }
...
gfc_match_decl_type_spec (gfc_typespec *ts, int implicit_flag)
{
  ...
  /* LINEAGE */
  if ((matched_type && strcmp ("vector", name) == 0)
      || (!matched_type && gfc_match (" vector") == MATCH_YES))
    {
      ts->type = BT_VECTOR;
      ts->kind = gfc_default_vector_kind;
      goto get_kind;
    }

  if ((matched_type && strcmp ("complexvec", name) == 0)
      || (!matched_type && gfc_match (" complexvec") == MATCH_YES))
    {
      ts->type = BT_COMPLEXVEC;
      ts->kind = gfc_default_complexvec_kind;
      return MATCH_YES;
    }
  /* LINEAGE */
...
gfc_match_data_decl (void)
{
  ...
  fprintf (stderr, "gfc_match_data_decl in\n");
...
cleanup:
  gfc_free_array_spec (current_as);
  current_as = NULL;
  fprintf (stderr, "gfc_match_data_decl success\n");
  return m;
}
