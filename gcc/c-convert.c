...
convert (tree type, tree expr)
{
  ...
  switch (code)
    ...
    {
      /* 
	   * For lineage, we do the following:
	   * Pack REAL_TYPE and INTEGER_TYPE into vectors
	   */
      if (TREE_CODE (TREE_TYPE (e)) == REAL_TYPE) {
        tree re = e;
        tree ve;

        if (TYPE_PRECISION (TREE_TYPE (e)) == 
			TYPE_PRECISION (float_type_node)) 
		{
          re = convert_to_real (double_type_node, e);
        }

        ve = build_vector_from_val (type, re);
        ret = convert_to_vector (type, ve);
      } else if (TREE_CODE (TREE_TYPE (e)) == INTEGER_TYPE) {
        /* For constant expr e, build_vector_from_val will generate VECTOR_CST
         * directly.  */
        if (CONSTANT_CLASS_P (e)) {
          tree re = convert_to_real (double_type_node, e);
          ret = build_vector_from_val (type, re);
        } else {
          tree re = convert_to_real (double_type_node, e);
          tree ve = build_vector_from_val (type, re);
          ret = convert_to_vector (type, ve);
        }
      } else {
