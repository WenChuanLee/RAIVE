      ...
      bool nested, location_t locus)
{
  ...
  if (name && !strcmp (IDENTIFIER_POINTER (name), "__builtin_ia32_sqrtpd256"))
  {
...
#if 0
  fprintf (stderr, "c_builtin_function: name=[%s]\n", name);
#endif
	    ...
	    bool initialized, tree attributes)
{
  ...
  /* Only VAR_DECL comes here. PARM_DECL goes to push_parm_decl. */

  if (flag_lineage)
  {
    if (TREE_CODE (decl) == VAR_DECL
        && TREE_TYPE (decl) && TYPE_SIZE (TREE_TYPE (decl)))
    {
      fprintf (stderr, "in start_decl: size=%lu ", (TREE_INT_CST_LOW (TYPE_SIZE (TREE_TYPE (decl)))));
      //debug_tree (decl);
      fprintf (stderr, "\n");
      ...
      if (flag_lineage_vec)
      {
        ...
        {
          TREE_TYPE (decl) = double_type_node;
          relayout_decl (decl);
          fprintf (stderr, "Reset to double: ");
          //debug_tree (decl);
          fprintf (stderr, "\n");
        }
      }
...
push_parm_decl (const struct c_parm *parm, tree *expr)
{
  ...
  if (flag_lineage)
  {
    if (TREE_CODE (decl) == PARM_DECL)
    {
      fprintf (stderr, "in push_parm_decl: ");
      //debug_tree (decl);
      fprintf (stderr, "\n");
